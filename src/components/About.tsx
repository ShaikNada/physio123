...
const About = () => {
  ...
  return (
    <section id="about" className="section-padding bg-white py-20 px-4 lg:px-16">
      <div className="container mx-auto">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="relative animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <div
              ref={scrollRef}
              className="overflow-x-auto no-scrollbar flex space-x-6 w-full max-w-full"
            >
              {experts.map((expert, idx) => (
                <div
                  key={idx}
                  className="min-w-[260px] bg-white rounded-xl shadow-md p-4 text-center group relative transition-transform duration-300 hover:scale-105"
                >
                  <div className="relative w-full h-72 overflow-hidden rounded-xl">
                    ...
                    />
                    <div className="absolute inset-0 bg-black bg-opacity-60 opacity-0 group-hover:opacity-100 transition duration-300 flex items-center justify-center text-white text-sm px-4 text-center">
                      {expert.info}
                    </div>
                  </div>
                  <h4 className="mt-4 text-lg font-semibold text-gray-800">{expert.name}</h4>
